...
function main() {
  ...
  if (!file) {
    console.error('Error: missing file path. Usage: node scripts/validate_link_strategy_brief_json.js brief.json');
    return 2;
  }
  ...
  if (!Array.isArray(data.target_pages) || data.target_pages.length === 0) {
    console.error('Invalid: target_pages must be a non-empty array.');
    return 1;
  }
  ...
  console.log('OK: link strategy brief passed validation.');
  return 0;
}
